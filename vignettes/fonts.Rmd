...
that SVG renderers know what fonts to use. This means that for optimal
display, the font must be available on both the computer used to
create the svg, and the computer used to render the svg. The defaults
are fonts that are available on almost all systems: there may be small
differences between them, but they are unlikely to cause problems in
most causes.

| R face   | Font on Windows    | Font on Unix |
|----------|--------------------|--------------|
| `sans`   | Arial              | Arial        |
| `serif`  | Times New Roman    | Times        |
| `mono`   | Courier            | Courier      |
| `symbol` | Standard Symbols L | Symbols      |

One downside to these default fonts is that they do not have good
coverage of characters for non-latin alphabets. If the placement of
graphical elements in your plot looks odd (text not perfectly
centered, etc), chances are it is because the font used by svglite for
computing text metrics does not support some characters. This can be
fixed by using the arguments `system_fonts` and `user_fonts` which
provide control over which fonts to use during SVG generation and
rendering.
...
space). The `user_fonts` arguments takes either paths to font files,
fonts from the `fontquiver` package, or a list that specifies the
alias. Whereas `system_fonts` gets a named list of families as argument,
`user_fonts` takes a named tree of lists of families (`sans`, `serif`,
`mono` and `symbol`) and faces (`plain`, `italic`, `bold`,
`bolditalic`, `symbol`):
...
You can also control which fonts gets written in the `font-family`
...
combinations of standard R families and faces. Symbola provides a
reproducible symbol font. The fontquiver function `font_families()`
produces a list with the appropriate structure and can be directly
supplied to svglite:
...
The C library Fontconfig is used as backend to find fonts installed on
the system. Fontconfig is the standard interface to system fonts on
Linux systems. It is also provided on OSX alongside the X11 graphics
system and comes with R on Windows. A warning is issued if Fontconfig
cannot find a system font. The gdtools package provides tools in case
you have trouble with a particular font:
...
Sometimes Fontconfig cannot find a font because it is not configured
correctly. Use the `debug` argument to find out which configuration
files Fontconfig is currently using:
...
for more information on configuring Fontconfig.
