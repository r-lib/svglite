...
svglite's device functions (with 10x8 the default). Altough those
dimensions are supplied in inches, the viewBox's user coordinate
...
rescaled to fit the container (altough Internet Explorer currently
...
* <https://www.stat.auckland.ac.nz/~paul/Reports/gridSVGsizing/sizing.html>
...
you supply to the device functions (10x8 being the default). Altough
those dimensions are specified in inches, the coordinate system is
scaled in points. Counting 72 points per inch, the default SVG surface
is thus 720x576 pt. Note that the CSS standard defines 12pt to be
equal to 16px, the default size of text in most browsers. Since 12pt
is the default text size in svglite as well, a SVG scaled to its
natural dimensions will appear seamless with web text of 16px. If the
text in your web page has another size, you will have to compute a
scale factor and adjust the dimensions of the SVG accordingly.
...
A default svglite plot surface is thus 720x576 pt.
